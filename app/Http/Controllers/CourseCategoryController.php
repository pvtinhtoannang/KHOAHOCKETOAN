...
class CourseCategoryController extends Controller
{
    ...
    {
        $get_term = $this->taxonomy->parent_id($parent)->name($this->tax)->get();
    ...
    {
        return view('admin.course.course-category.course-cat', ['categories' => $this->fetchCategoryTree()]);
    }
        ...
        if ($request->category_name != null && $request->category_slug != null) {
            if ($this->term->slug($request->category_slug)->first() == null) {
                ...
                if ($term) {
                    ...
                    return redirect()->route('GET_COURSE_CATEGORY_ROUTE')->with('messages', 'success')->withInput();
                } else {
                    return redirect()->route('GET_COURSE_CATEGORY_ROUTE')->with('messages', 'error')->withInput();
                }
            } else {
                return redirect()->route('GET_COURSE_CATEGORY_ROUTE')->with('messages', 'error')->withInput();
            }
